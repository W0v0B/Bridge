...
- **滚动到底部按钮**（↓ 图标）：自动滚动暂停时出现；点击可刷新最新缓冲输出并恢复自动滚动。
...
- **自动滚动暂停**：通过鼠标滚轮向上滚动或拖动滚动条可暂停自动滚动，并完全暂停 DOM 更新——数据继续在 `htmlStringMap` 中累积，直至达到配置的行数上限。恢复时（滚动到底部或点击 ↓ Bottom），触发一次 `flushToDOM` 追赶更新。切换设备时自动滚动重置为开启状态。
